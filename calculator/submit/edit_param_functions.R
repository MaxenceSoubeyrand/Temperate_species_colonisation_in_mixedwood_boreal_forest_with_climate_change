...
create_param_file <- function(data, file_name, growth, timesteps, out_dir, par_dir,
                              plot_size=100, tree_map=F, tree_map_name=NA) {
    ...
    #If tree_maps then put the link in the paramater file:
    if(tree_map){
    ...
    for(i in 1:n){
        ...
        growth_sp <- growth$growth_no_biais[which(growth$species==sp)] #La croissance de la ième espèce dans le plot
