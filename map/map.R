...
FERLD <- read.csv("map/FERLD_climateNA.csv") %>% 
  mutate(plot_name="")

dom_bio <- readOGR(dsn = "map/sous_dom_bioclim.shp",
                   layer = "sous_dom_bioclim")

lim_ers <- readOGR(dsn = "map/ERS/ERS_northern_boundary.shp",
                   layer = "ERS_northern_boundary")
lim_ers <- spTransform(lim_ers, CRS("+proj=longlat +ellps=WGS84 +datum=WGS84"))

lim_err <- readOGR(dsn = "map/ERR/ERR_northern_boundary.shp",
                   layer = "ERR_northern_boundary")
lim_err <- spTransform(lim_err, CRS("+proj=longlat +ellps=WGS84 +datum=WGS84"))

lim_boj <- readOGR(dsn = "map/BOJ/BOJ_northern_boundary.shp",
...
pdf(file="figure/map_ggplot.pdf")
  map
dev.off()
